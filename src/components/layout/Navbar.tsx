...
export default function Navbar() {
  ...
  return (
    <header className="absolute top-2 md:top-4 left-0 right-0 z-100 flex justify-center items-center px-6 md:px-10 py-2">
      ...
      {/* Menú lateral móvil */}
      {menuOpen && (
        <div className="fixed inset-0 z-50 w-2xs bg-teal-50 text-teal-950 flex flex-col items-start px-6 py-6 animate-slide-in overflow-y-auto">
          {/* Botón cerrar */}
          <button
         ...
         {/* Navegación móvil */}
          <nav className="flex flex-col gap-4 text-lg w-full font-medium">
            ...
            <a href="#" onClick={() => setMenuOpen(false)} className="hover:text-sky-300 transition">Servicios</a>
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="justify-center flex items-center gap-1 border-1 px-4 py-2 rounded-lg transition hover:bg-teal-100 hover:text-teal-900 "
              onClick={() => setMenuOpen(false)}
            >
              <FaWhatsapp className="text-2xl" />
              WhatsApp
